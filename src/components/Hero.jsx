...
function Hero() {
  ...
  // --- 1. DEVASA ŞEHİR LİSTESİ (TAM LİSTE - BURADA DA OLMALI) ---
  ...
  const sendQuote = async (e) => {
    ...
    try {
        await fetch("https://localhost:7142/api/Quotes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(backendData) });
  ...
  const inputStyle = { width: '100%', padding: '12px', marginBottom: '10px', border: '1px solid #ccc', borderRadius: '8px' };
  const labelStyle = { display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '14px', color:'#0a1a2f' };
  const buttonStyle = { width: '100%', padding: '15px', background: 'linear-gradient(135deg, #d90429, #a8001d)', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer', marginTop: '10px' };
  ...
  return (
    <div style={heroSectionStyle}>
      <div style={contentContainer}>
        ...
        <div style={textSide}>
          <h1 style={{ fontSize: '3.5rem', fontWeight: '900', lineHeight: '1.2', marginBottom: '20px' }}>
            PROFESYONEL <br /> <span style={{ color: '#ffb703' }}>EVDEN EVE</span> <br /> NAKLİYAT
          </h1>
          <p style={{ fontSize: '1.2rem', marginBottom: '30px', color: '#ddd' }}>
            Türkiye'nin 81 iline sigortalı, asansörlü ve ambalajlı taşımacılık hizmeti. Eşyalarınız bize emanet.
          </p>
          <div style={{ display: 'flex', gap: '20px' }}>
             <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
                <div style={{width:'50px', height:'50px', background:'#d90429', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.5rem'}}>🛡️</div>
                <div><strong>Sigortalı</strong><br/><span style={{fontSize:'0.8rem'}}>Taşıma</span></div>
             </div>
             <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
                <div style={{width:'50px', height:'50px', background:'#d90429', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.5rem'}}>🏗️</div>
                <div><strong>Asansörlü</strong><br/><span style={{fontSize:'0.8rem'}}>Hizmet</span></div>
             </div>
          </div>
        </div>

        {/* SAĞ TARAFTAKİ FORM (Senin Formun) */}
        <div style={formSide}>
          <h2 style={{ textAlign: 'center', color: '#0a1a2f', borderBottom: '3px solid #f4f6f9', paddingBottom: '15px', marginBottom:'20px' }}>Hemen Fiyat Al ⚡</h2>
          ...
          <form ref={form} onSubmit={sendQuote}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <div style={{ flex: 1 }}><label style={labelStyle}>Ad Soyad</label><input style={inputStyle} type="text" name="user_name" value={formData.user_name} onChange={handleChange} placeholder="Adınız" required /></div>
              <div style={{ flex: 1 }}><label style={labelStyle}>Telefon</label><input style={inputStyle} type="tel" name="user_phone" value={formData.user_phone} onChange={handleChange} placeholder="0555..." required /></div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <div style={{ flex: 1 }}><label style={labelStyle}>Mevcut Kat</label><input style={inputStyle} type="text" name="from_floor" value={formData.from_floor} onChange={handleChange} placeholder="Örn: 3" /></div>
              <div style={{ flex: 1 }}><label style={labelStyle}>Yeni Kat</label><input style={inputStyle} type="text" name="to_floor" value={formData.to_floor} onChange={handleChange} placeholder="Örn: 5" /></div>
            </div>
...
<input 
  ...
  style={inputStyle} 
  placeholder="Örn: 2+1 veya Özel Eşyalar..."
/>
<datalist id="room-options">
  ...
  <option value="4+1" />
  <option value="Villa" />
  <option value="Ofis Taşıma" />
  <option value="Parça Eşya" />
  <option value="Öğrenci Evi" />
</datalist>
